...
func TestCliTool_InvokableRun_SuccessArgsAndEnvPrecedence(t *testing.T) {
	...
	args := map[string]any{"args": []string{"ARGVAL"}}
...
func TestCliTool_InvokableRun_EmptyCommandError(t *testing.T) {
	ctx := context.Background()
	tool := &CliTool{Def: Definition{Command: ""}}
...
func TestCliTool_InvokableRun_WorkdirUsed(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
    tool := &CliTool{Def: MustNewDefinition("pwd", "/bin/sh -c pwd", "", nil), Workdir: dir}
	resp, err := tool.InvokableRun(ctx, "{}")
	require.NoError(t, err)
    var m map[string]any
    require.NoError(t, json.Unmarshal([]byte(resp), &m))
...
func TestCliTool_InvokableRun_EmptyArgumentsString(t *testing.T) {
    ...
    // Pass empty arguments string; implementation should treat it as "{}"
    resp, err := tool.InvokableRun(ctx, "")
    require.NoError(t, err)
    var m map[string]any
    require.NoError(t, json.Unmarshal([]byte(resp), &m))
